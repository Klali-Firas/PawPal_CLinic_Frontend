...
import { Animaux, Services, ServicesAnimaux, Utilisateurs, RendezVous } from 'src/app/interfaces/interfaces';
import { AnimauxService } from 'src/app/services/animaux.service';
import { ServiceService } from 'src/app/services/service.service';
import { ServiceAnimauxService } from 'src/app/services/serviceanimaux.service';
...
export class RendezvousComponent implements OnInit {
  ...
  constructor(
    ...
    private serviceAnimauxService: ServiceAnimauxService,
  ...
  onSubmit(): void {
    if (this.rendezvousForm.valid) {
      ...
      const rendezVous: RendezVous = {
        ...
        statut: 'en_attente',
        motif: selectedService?.nomService,
        creeLe: new Date(),
      };

      this.rendezvousService.createRendezVous(rendezVous).subscribe({
        next: (createdRendezVous: RendezVous) => {
          const serviceAnimaux: ServicesAnimaux = {
            id: 0, // Assuming the ID will be generated by the backend
            animalId: formValue.animalId,
            veterinaireId: null,
            serviceId: formValue.motif,
            dateService: new Date(formValue.dateRendezVous),
            remarques: null,
          };

          this.serviceAnimauxService.createServicesAnimaux(serviceAnimaux).subscribe({
            next: (response: ServicesAnimaux) => {
              this.toaster.success('Rendez-vous and service created successfully');
              this.rendezvousForm.reset();
              this.rendezvousForm.patchValue({
                proprietaire: this.user.nom + ' ' + this.user.prenom
              });
            },
            error: (error: any) => {
              console.error('Error creating service', error);
              this.toaster.error('Error creating service');
            }
          });
        },
        error: (error: any) => {
